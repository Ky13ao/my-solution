...
- Chúng ta có thể đưa dữ liệu vào 2 loại Data Sink:
  ...
  - **Kafka Sink**:
    ...
    - tuy nhiên, bạn sẽ cần phải chuyển đổi dữ liệu từ dạng Dataframe
...
- Để giải quyết vấn đề trùng lặp dữ liệu ở trên,
  bạn có thể sử dụng Streaming Watermark.
...
- Spark cũng hỗ trợ cơ chế Left Join và Right Join
  ...
  tuy nhiên bạn sẽ cần chú ý một số điều như sau:
  - Khi sử dụng **Left Join** thì Streaming Dataframe sẽ phải nằm ở bên **trái**,
    ngược lại nếu bạn sử dụng **Right Join** thì Streaming Dataframe
...
- Tuy nhiên, cũng cần lưu ý rằng việc sử dụng Stateful Transformations
  cũng sẽ tốn rất nhiều bộ nhớ, vậy nên bạn phải có phương án quản lý, thiết kế
...
- Ví dụ về kết quả của _Tumbling Window_ như sau:
  ...
  +-------------------+-------------------+--------+---------+
- Cơ chế này cũng có một hạn chế, đó là bạn chỉ có thể kết hợp dữ liệu
...
- Kể cả khi dữ liệu đến muộn thì Spark vẫn có thể xử lý do có cơ chế lưu trữ tại
  ...
  - **Output mode**:
    - Bạn nên sử dụng Output mode hợp lý, ví dụ như ở mode "complete" thì
...
- **Tumbling Window** có 1 đặc điểm là các khoảng thời gian sẽ không được đè lên
  ...
  5 phút, lúc này bạn sẽ cần sử dụng **Sliding Window** thay cho
...
- Khi phát triển ứng dụng Spark, ta có thể gặp các lỗi sau:
  - Lỗi từ User Code: Lỗi liên quan đến các vấn đề từ đoạn code mà bạn đã viết
...
- Tài nguyên: [Dataset](https://drive.google.com/drive/folders/1uq4TNKlSE-a_UuVSUSidartcUtRGmS30?usp=sharing)
  ...
  - **Answer.csv**
    - File csv chứa các thông tin liên quan đến câu trả lời và có cấu trúc như sau: - _Id_: Id của câu trả lời. - _OwnerUserId_: Id của người tạo câu trả lời đó.
      (Nếu giá trị là NA thì tức là không có giá trị này) - _CreationDate_: Ngày câu trả lời được tạo. - _ParentId_: ID của câu hỏi mà có câu trả lời này. - _Score_: Điểm số mà người trả lời nhận được từ câu trả lời này. - _Body_: Nội dung câu trả lời.

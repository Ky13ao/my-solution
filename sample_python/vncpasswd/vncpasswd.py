...
__credits__ = ['Yusuke Shinyama', 'Richard Outerbridge',
               'Dan Hoey', 'Jim Gillogly', 'Phil Karn']
__license__ = "MIT"
__version__ = "1.2.3"
...
if platform.system().startswith('Windows'):
    from WindowsRegistry import WindowsRegistry as wreg
...
def unhex(s):
    ...
    try:
        ...
        if e.message == 'Odd-length string':
            eprint('WARN: %s . Chopping last char off... "%s"' %
                   (e.message, s[:-1]))
            s = s[:-1].decode('hex')
        else:
            raise
...
def run_tests(verbose=False):
    ...
    ignore_methods = ['__builtins__', '__doc__', '__file__', '__name__',
                      '__package__', '__warningregistry__', 'argparse', 'sys']
    methods = [
        i for i in methods if not i in ignore_methods or ignore_methods.remove(i)]
...
def main():
    ...
    if (args.registry and args.decrypt and platform.system().startswith('Windows')):
        ...
        eprint('Cannot read from Windows Registry on a %s system' %
               platform.system())
    ...
    if (len(hexpasswd) > 16 and (len(hexpasswd) % 16) == 0):
        eprint(
            'INFO: Detected ciphertext > 64 bits... breaking into blocks to decrypt...')
    ...
    if (args.registry and not args.decrypt and platform.system().startswith('Windows')):
        ...
        eprint('WARN: Cannot write to Windows Registry on a %s system' %
               platform.system())
